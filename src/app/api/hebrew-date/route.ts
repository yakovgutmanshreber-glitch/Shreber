...
import { HDate, HebrewCalendar, Locale, gematriya, flags } from "@hebcal/core";
...
// GET /api/hebrew-date — today's Hebrew date, month, year + this week's parsha (or holiday).
export async function GET() {
  ...
  return NextResponse.json({
    date: clean(hd.renderGematriya()),
    label,
    parsha: label, // "פרשת …" (or the holiday name that week)
    month: clean(Locale.gettext(hd.getMonthName(), "he")),
    year: clean(gematriya(hd.getFullYear())),
  });
}
